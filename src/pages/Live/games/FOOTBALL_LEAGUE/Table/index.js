      ...
      <div className={style.right}>
        <Slider {...init}>
          {data.league.matches[0].odds[0].groups
            .slice(0, data.league.matches[0].odds[0].groups.length - 1)
            .map((el_o, idx_o) => (
              <div key={idx_o} className={style.slide}>
                <div className={style.head}>
                  {data.league.matches[0].odds[0].groups[idx_o].markets.map(
                    (el_ma, idx_ma) => (
                      <div key={idx_ma} className={style.column}>
                        {el_ma.name && (
                          <div className={style.legend}>
                            {el_ma.name.replaceAll('_', ' ')}
                          </div>
                        )}
                ...
                <div className={style.results}>
                  {data.league.matches.map((el_m, idx_m) => (
                    <div key={idx_m} className={style.row}>
                      {el_m.odds[0].groups[idx_o].markets.map(
                        (el_ma, idx_ma) => (
                          <div key={idx_ma} className={style.column}>
          ...
          {data.league.matches[0].odds[0].groups[7].markets.map(
            (el_ma, idx_ma) => (
              <div key={idx_ma} className={style.slide}>
                <div className={style.head}>
                  {el_ma.name && (
                    ...
                    >{`${el_ma.name.replaceAll('_', ' ')} ${el_ma.headers[0]}`}</div>
                  )}
                  {el_ma.outcomes.map((el, idx) => (
                ...
                <div className={style.results}>
                  {data.league.matches.map((el_m, idx_m) => (
                    <div key={idx_m} className={style.row}>
                      {el_m.odds[0].groups[7].markets[idx_ma].outcomes.map(
                        (el, idx) => (
                          <div key={idx} className={style.column}>
                            <div
                              className={classNames(
