...
const League = () => {
    ...
    useEffect(() => {
        fetchData(`https://stats.fn.sportradar.com/betradar/${getLocalStorage('i18nextLng')}/Europe:Helsinki/gismo/stats_season_tables/${url.league}`).then((data) => {
            setData(data)
            setLoading(false)
        })
    }, []);
                                                       ...
                                                       </div>
                                                       {
                                                           el.tablerows.sort((a, b) => { return a[sort] - b[sort] }).map((el, idx) =>
                                                               <NavLink
                                                                   key={idx}
                                                                   className={style.row}
                                                                   to={`/${url.id}/${url.category}/${url.league}/team/${el.team.uid}`}
                                                               >
                                                                   <div className={style.cell}>
