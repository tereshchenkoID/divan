            ...
            <div key={idx_c} className={style.column}>
              {el_c.map((el_i, idx_i) => (
                <div key={idx_i} className={style.cell}>
                  <Odd market={data.race.odds.markets[6].printname} start={data.start} data={el_i} view={'vertical'} roundId={data.race.id} />
                </div>
              ))}
            </div>
          ))}
        {sort.length > 0 &&
          sort.length !== NUMBER * 2 &&
          data.race.odds.markets[6].outcomes.map(
            (el, idx) =>
              sortBy(el, sort) && (
                <div key={idx} className={style.cell}>
                  <Odd market={data.race.odds.markets[6].printname} start={data.start} data={el} view={'vertical'} roundId={data.race.id} />
                </div>
              ),
          )}
      </div>
